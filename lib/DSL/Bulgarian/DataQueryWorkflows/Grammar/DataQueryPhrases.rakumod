...
role DSL::Bulgarian::DataQueryWorkflows::Grammar::DataQueryPhrases {
    ...
    token order-verb:sym<Bulgarian> { :i 'нареди' | ([\w]+) <?{ $0.Str !(elem) <зареди подреди> and is-bg-fuzzy-match($0.Str, 'нареди', 2) }> }
    ...
    token sort-verb:sym<Bulgarian> { :i 'сортирай' | ([\w]+) <?{ is-bg-fuzzy-match($0.Str, 'сортирай', 2) }> | 'нареди' | ([\w]+) <?{ $0.Str !(elem) <зареди подреди> and is-bg-fuzzy-match($0.Str, 'нареди', 2) }> }
